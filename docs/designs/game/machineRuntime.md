...
- `STORAGE_SPAWN_INTERVAL_MS=1000`, `MACHINE_PROCESS_TIME_MS=1000`, `MACHINE_INPUT_CAPACITY=6`, `MACHINE_OUTPUT_CAPACITY=1`。
...
- `AdvanceMachineRuntimeInput`: runtime, deltaMs, nowMs, 任意 config/recipes、必須 ID factory。
- `ExtractMachineOutputInput`: runtime, 出力接続一覧, 占有接続 ID Set。
...
process 中なら同一参照。消費可能レシピなしも同一参照。あれば必要入力を除き、`{recipeId, outputFoodId, remainingMs:1000}` を process にする。

### MachineRuntime advanceStorage(input)（内部）

spawnFoodId が未設定/生成不可なら elapsed を0にする。設定済みなら elapsed+deltaMs。1000未満は蓄積のみ。到達時は必ず ID factory を呼んで nowMs の item を作り、elapsed から1000を1回だけ引く。出力が1件以上なら新 item を破棄し既存 buffer を維持、空なら追加する。大きい delta でも1呼出で最大1生成。
...
未占有接続がなければ null。splitter 以外は未占有の先頭。splitter は `roundRobinIndex % outputConnections.length` から元接続順を巡回し、最初の未占有を返す。
